...
export const firebaseController = {
  ...
  async getThoughtsByUserId(): Promise<Thought[]> {
    // using the userId, get all thoughts by that user
    const auth = getAuth();
    const user = auth.currentUser;
    ...
    try {
      ...
      if (voteDoc.exists()) {
        ...
        if (newVoteType == "" && existingVote != "") {
          ...
          // User is voting from neutral state with existing vote card created
        ...
        // User is voting for the first time, new vote card
        // console.log("new vote card")
        if (newVoteType === "upvote") {
          await setDoc(
            thoughtRef,
            { karma: thought.karma + 1 },
